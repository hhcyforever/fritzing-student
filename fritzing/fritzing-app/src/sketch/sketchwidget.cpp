...
#include "../utils/cursormaster.h"
#include "../Singleton.h"
#include "../new_dialogs/dialog_additem.h"
...
{
    connect(this,SIGNAL(addResistor()),this,SLOT(showAddResistor()));
    Wparent = parent;
    if(viewID == 1){
        sleepTimer = new QTimer(this);
        connect(sleepTimer,SIGNAL(timeout()),this,SLOT(sleepNotify()));
        sleepTimer->setInterval(10000);
        sleepTimer->start();
        sleepFlag = 0;
    }
...
ItemBase * SketchWidget::addItemAux(ModelPart * modelPart, ViewLayer::ViewLayerPlacement viewLayerPlacement, const ViewGeometry & viewGeometry, long id, bool doConnectors, ViewLayer::ViewID viewID, bool temporary)
{
	...
	if (wire) {
        ...
        DebugDialog::debug(QString("addwire"));
        if(m_viewID == 1){
            Singleton::GetInstance()->emitPairsSVG("wire",newItem->id());
            Singleton::GetInstance()->emitInitPairs("wire",newItem->id());
            DebugDialog::debug(QString::number(newItem->id()));
        }
		return wire;
	}
    ...
    if(m_viewID == 1){
        ...
        PaletteItemBase * paletteItemBase = qobject_cast<PaletteItemBase *>(newItem);
        Singleton::GetInstance()->emitInitPairs(newItem->title(),newItem->id());
        if (paletteItemBase != NULL) {
//                qDebug("sketch palette"+paletteItemBase->filename().toUtf8());
                qDebug("sketch item"+newItem->title().toUtf8());
                QString file = paletteItemBase->filename();
                QString filename = file.mid(file.indexOf("breadboard/")+11);
                Singleton::GetInstance()->emitPairsSVG(filename,newItem->id());

                }
            }
    if(m_viewID == 2){
        if(newItem->title() == "220Ω Resistor"){
            QTimer::singleShot(500,this,SLOT(showAddResistor()));
    }
    }
...
void SketchWidget::deleteItem(ItemBase * itemBase, bool deleteModelPart, bool doEmit, bool later)
{
    ...
    //if(m_viewID == 1) Singleton::GetInstance()->emitdata(id,itemBase->title(),false);
...
void SketchWidget::sleepNotify(){
        sleepFlag = 1;
        //qDebug("sleep");
        Singleton::GetInstance()->emitsleep();
}
...
void SketchWidget::mouseMoveEvent(QMouseEvent *event) {
    ...
    {
        sleepFlag = 0;
        Singleton::GetInstance()->emitwake();
        //qDebug("sleepTimer stop");
    }
    else
    {
									...
									bool connect, bool updateConnections)
{
    ...
    if(m_viewID == 1){
        Singleton::GetInstance()->analyseData(fromID,fromConnectorID,toID,toConnectorID,connect);
        Singleton::GetInstance()->judgeIntersect();
    }
	ItemBase * fromItem = findItem(fromID);
	if (fromItem == NULL) {
...
//void SketchWidget::emitdata(long ID, QString ConnectorID, bool addordelete){
//    QString s = QString::number(ID);
//    QByteArray bytes1 = s.toUtf8();
...
//    io->socket()->Emit("change message",msg1,msg2,msg3); //发送"ID message"事件
//    io->socket()->Emit("location message",msg2);//发送"location message"事件
//    io->socket()->Emit("bool message",msg3);//发送"bool message"事件
//}

//void SketchWidget::emitInitPairs(QString constTitle,qint64 id){
//    QString id_s = QString::number(id);
//    QByteArray bytes1 = id_s.toUtf8();
//    std::string msg1(bytes1.data(),bytes1.length());
//    QByteArray bytes2 = constTitle.toUtf8();
//    std::string msg2(bytes2.data(),bytes2.length());
//    io->socket()->Emit("Init title",msg2);
//    io->socket()->Emit("Init id",msg1);
//}
void SketchWidget::showAddResistor(){
    qDebug("add resistor");
    dialog_additem addresistor(this);
    addresistor.setAttribute(Qt::WA_ShowModal, true);
    addresistor.exec();
}
