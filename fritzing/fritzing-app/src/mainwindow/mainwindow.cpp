...
#include "../new_dialogs/shadowdlg.h"
#include "../new_dialogs/login.h"
#include "../Singleton.h"
...
vector<simple_item> itemsStruct;
vector<simple_item> itemsStruct2;
    ...
    FritzingWindow(untitledFileName(), untitledFileCount(), fileExtension(), parent)
{
    ...
    login loginNow(this);
    loginNow.setAttribute(Qt::WA_ShowModal, true);
    loginNow.exec();

    connect(this,SIGNAL(MsgSignal(QString)),this,SLOT(showMsg(QString)));
    connect(this,SIGNAL(PraiseSignal(QString)),this,SLOT(showPraise(QString)));
    connect(this,SIGNAL(HighlightSignal(qreal,qreal,qreal,qreal)),this,SLOT(showShadow(qreal,qreal,qreal,qreal)));
    using std::placeholders::_1;
    using std::placeholders::_2;
    using std::placeholders::_3;
    using std::placeholders::_4;
    socket::ptr sock = s->io->socket("/student");
    BIND_EVENT(sock,"praise",std::bind(&MainWindow::OnNewPraise,this,_1,_2,_3,_4));
    BIND_EVENT(sock,"text",std::bind(&MainWindow::OnNewMessage,this,_1,_2,_3,_4));
    BIND_EVENT(sock,"highlight",std::bind(&MainWindow::OnNewHighlight,this,_1,_2,_3,_4));
    
//    QTimer *testTimer = new QTimer(this);
//    connect(testTimer, SIGNAL(timeout()), this, SLOT(showShadow2()));
//    testTimer->start(50000);

    QTimer *screenShotTimer = new QTimer(this);
    connect(screenShotTimer, SIGNAL(timeout()), this, SLOT(slotGrabWidgetScreen()));
    screenShotTimer->start(100);


    //connect(this, SIGNAL(mousePressSignal(QMouseEvent*)), this, SLOT(mouseMoveNotify(QMouseEvent*)));
...
void MainWindow::OnNewMessage(std::string const& name,message::ptr const& data,bool hasAck,message::list &ack_resp){
    ...
    qDebug("message:"+ QString::fromStdString(data->get_string()).toUtf8());
    emit MsgSignal(QString::fromStdString(data->get_string()));
}

void MainWindow::OnNewPraise(std::string const& name,message::ptr const& data,bool hasAck,message::list &ack_resp){
    std::string a = "praise:";
    qDebug("praise:"+ QString::fromStdString(data->get_string()).toUtf8());
    emit PraiseSignal(QString::fromStdString(data->get_string()));
}

void MainWindow::OnNewHighlight(std::string const& name,message::ptr const& data,bool hasAck,message::list &ack_resp){
    std::string a = "praise:";
    qDebug("show shadow");

//    std::vector<std::shared_ptr<sio::message>> vct = data->get_vector();

    qreal n1 = data->get_map()["x"]->get_double();
    qreal n2 = data->get_map()["y"]->get_double();
    qreal n3 = data->get_map()["width"]->get_double();
    qreal n4 = data->get_map()["height"]->get_double();
    qDebug("1:"+ QString::number(n1).toUtf8());
    qDebug("2:"+ QString::number(n2).toUtf8());
    qDebug("3:"+ QString::number(n3).toUtf8());
    qDebug("4:"+ QString::number(n4).toUtf8());
    emit HighlightSignal(n1,n2,n3,n4);
}

void MainWindow::showMsg(QString a){
    qDebug("show message");
    MsgDlg dlg(Msg_msg,a,this->window());
    dlg.setAttribute(Qt::WA_ShowModal, true);
    dlg.startTimer();
    dlg.exec();
    //QMessageBox::information(this, QString::fromStdString("praise"),QString::fromStdString("wonderful"));
}

void MainWindow::showPraise(QString a){
    qDebug("show praise");
    MsgDlg dlg(Msg_Praise,a,this->window());
    dlg.setAttribute(Qt::WA_ShowModal, true);
    dlg.startTimer();
    dlg.exec();
}

void MainWindow::showShadow(qreal n1, qreal n2, qreal n3, qreal n4){
    qDebug("show shadow now");
    int width = (int)(this->window()->width()*0.7394);
    int height = (int)(this->window()->height()*0.7659);

    int x = (int)(n1*width);
    int y = (int)(n2*height+this->window()->height()*0.2);
    int wid = (int)(n3*width);
    int heig = (int)(n4*height);
    qDebug("11:"+ QString::number(x).toUtf8());
    qDebug("22:"+ QString::number(y).toUtf8());
    qDebug("33:"+ QString::number(wid).toUtf8());
    qDebug("44:"+ QString::number(heig).toUtf8());
    shadowdlg shadow_dlg(x,y,wid,heig,this->window());
    qDebug(QString::number(this->window()->width()).toUtf8());
    qDebug(QString::number(this->window()->height()).toUtf8());
    shadow_dlg.setAttribute(Qt::WA_ShowModal, true);
    shadow_dlg.startTimer();
    shadow_dlg.exec();
}

void MainWindow::showShadow2(){
    qDebug("show shadow now");
...
//    qDebug("33:"+ QString::number(wid).toUtf8());
//    qDebug("44:"+ QString::number(heig).toUtf8());
    shadowdlg shadow_dlg(100,200,400,200,this);
    qDebug(QString::number(this->width()).toUtf8());
    qDebug(QString::number(this->height()).toUtf8());
    shadow_dlg.setAttribute(Qt::WA_ShowModal, true);
    shadow_dlg.exec();
}
...
{
    int yoffset = (int)(this->height()*0.0993);
    int w = (int)(this->width()*0.7394);
    int h = (int)(this->height()*0.7659);
    QPixmap p = this->grab(QRect(0, yoffset, w, h));
    QString filePathName = "widget";
    filePathName += QDateTime::currentDateTime().toString("yyyy-MM-dd hh-mm-ss-zzz");
    filePathName += ".png";
    filePathName = "../fritzing-app/jpg_qt/"+filePathName;
    if(!p.save(filePathName,"png"))
    {
        qDebug("save widget screen failed");
    }
    QString pixstr = pixmapTostring(p);
    Singleton::GetInstance()-> emitImage(pixstr);
}


QString MainWindow::pixmapTostring( const QPixmap& pixmap )
{
//  QByteArray byte_array;
//  QBuffer bf(&byte_array);
//  pixmap.save(&bf,"png");
...
//    QString filename = "../fritzing-app/fzzs/Leds/Leds.fz";

//}

void MainWindow::loadTutorial(){
    const char* filename = "../fritzing-app/fzzs/Leds/Leds.fz";
    TiXmlDocument doc;
